...
export default function AdminDirectory() {
  ...
  const counts = MEMBER_STATUSES.reduce(
    (acc, item) => ({
      ...acc,
      [item]: members.filter((member) => member.status === item).length,
    }),
    {} as Record<MemberStatus, number>,
  );

  const applyUpdate = (id: string, updates: Partial<MemberProfile>) =>
    setMembers(updateMember(id, updates, adminSession));
  const remove = (id: string) =>
    window.confirm("Delete this profile permanently?") &&
    setMembers(deleteMember(id, adminSession));
  const exportCsv = () => {
    const blob = new Blob([toCsv(members, adminSession)], {
  ...
  return (
    <PageLayout>
        ...
        <div className="content-container flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-primary-foreground/75">
              Review pending profiles, approve public listings, and keep the
              directory clean.
            </p>
          </div>
                ...
                <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-start">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {member.title} · {member.company_name} · {member.industry}
                    </p>
                  ...
                  <div className="flex flex-wrap gap-2 lg:max-w-sm lg:justify-end">
                    <Button
                      ...
                      className="rounded-full"
                      onClick={() =>
                        applyUpdate(member.id, { status: "approved" })
                      }
                    >
                      Approve
                    </Button>
                    <Button
                      ...
                      className="rounded-full"
                      onClick={() =>
                        applyUpdate(member.id, { status: "rejected" })
                      }
                    >
                      Reject
                    </Button>
                    <Button
                      ...
                      className="rounded-full"
                      onClick={() =>
                        applyUpdate(member.id, { status: "hidden" })
                      }
                    >
          ...
          {visibleMembers.length === 0 && (
            <div className="mt-8 rounded-[1.75rem] border border-dashed border-border bg-muted/50 p-10 text-center">
              ...
              <p className="mt-3 text-muted-foreground">
                New submissions will appear here as pending.
              </p>
            </div>
          )}
        </div>
      ...
      <EditDialog
        ...
        onSave={(updated) => {
          setMembers(updateMember(updated.id, updated, adminSession));
          setEditing(null);
        }}
      />
    </PageLayout>
...
function StatusButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-2xl border p-4 text-left transition ${active ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card hover:border-primary/30"}`}
    >
      <span className="block text-2xl font-bold">{count}</span>
      <span className="mt-1 block text-sm font-medium capitalize opacity-80">
        {label}
      </span>
    </button>
  );
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto rounded-2xl">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Textarea
              rows={3}
              value={draft.areas_of_expertise}
              onChange={(e) => setField("areas_of_expertise", e.target.value)}
            />
          </div>
          <div className="space-y-2 md:col-span-2">
            ...
            <Textarea
              rows={3}
              value={draft.professional_interests}
              onChange={(e) =>
          ...
          <label className="flex items-start gap-3 rounded-2xl border border-border bg-muted/50 p-4 md:col-span-2">
            <Checkbox
              checked={draft.open_to_collaboration}
